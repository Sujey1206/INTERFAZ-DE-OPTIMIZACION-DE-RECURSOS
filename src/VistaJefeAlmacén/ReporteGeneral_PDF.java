...
public class ReporteGeneral_PDF {
    ...
    public ReporteGeneral_PDF(String nombre, String fecha, String rutaImagen) {
        this.nombre = nombre;
        this.fecha = fecha;
        this.rutaImagen = rutaImagen;
        documento = new Document();
        titulo = new Paragraph("REPORTE GENERAL DE LOS RECURSOS");
    }

    public ReporteGeneral_PDF(ConsultasRecursos modeloR) {
    ...
    public void crearPlantilla() {
        try {
            ...
            documento.add(new Paragraph("****************************************************************************************************************"));
            documento.add(Chunk.NEWLINE);            
            Paragraph texto = new Paragraph("El presente documento es un reporte de los recursos y/o materiales actuales en el Almacén de la empresa. Por este medio se da constancia del control realizado por el sistema CATO.");
            ...
            for (Recursos n : modeloR.listar()) {
                ...
                if(n.getCantrec()==0){
                    ...
                    tabla.addCell(obs1);
                }else if(n.getCantrec()<=10){
                    PdfPCell obs1 = new PdfPCell(new Phrase("EN ESCASEZ"));
                    obs1.setBackgroundColor(BaseColor.YELLOW);
                    tabla.addCell(obs1);
                }else{
                    PdfPCell obs1 = new PdfPCell(new Phrase("CON STOCK"));
                    obs1.setBackgroundColor(BaseColor.GREEN);
                    tabla.addCell(obs1);
                }
            }
